...
def _parse_fn(example):
    ...
    example_fmt = {
        ...
        "flow_u": tf.FixedLenFeature([], tf.string),
        "flow_v": tf.FixedLenFeature([], tf.string),
        "labels": tf.FixedLenFeature([], tf.int64)
    }
    parsed = tf.parse_single_example(example, example_fmt)
    rgb = tf.decode_raw(parsed["rgb"], tf.float32)
    rgb = tf.reshape(rgb, data_shape)
    u = tf.decode_raw(parsed["flow_u"], tf.float32)
    u = tf.reshape(u, data_shape)
    v = tf.decode_raw(parsed["flow_v"], tf.float32)
    v = tf.reshape(v, data_shape)
    return {'rgb': rgb, 'u': u, 'v': v}, parsed["labels"]


def input_fn(train_input_path, eval_input_path):
    # train dataset
    train_dataset = tf.data.TFRecordDataset(train_input_path)
    train_dataset = train_dataset.shuffle(buffer_size=int(num_train_samples/2))
...
def nn_classifier(train_input_path, eval_input_path, params):
    with tf.device('/cpu:1'):
        ...
        input_rgb = tf.reshape(data['rgb'], [-1, params['data_shape'][0], params['data_shape'][1],
                                             params['data_shape'][2]])
        input_u = tf.reshape(data['u'],
                             [-1, params['data_shape'][0], params['data_shape'][1],
                              params['data_shape'][2]])
        input_v = tf.reshape(data['v'],
                             [-1, params['data_shape'][0], params['data_shape'][1],
                              params['data_shape'][2]])
        input_rgb = tf.cast(input_rgb, tf.float32, name='input_rgb')
        input_u = tf.cast(input_u, tf.float32, name='input_u')
        input_v = tf.cast(input_v, tf.float32, name='input_v')

        learning_rate = tf.placeholder(dtype=tf.float32, name='learning_rate')
        mode = tf.placeholder(dtype=tf.bool, name='mode')

        with tf.variable_scope('conv1_rgb'):
            conv1_rgb = tf.layers.conv2d(inputs=input_rgb, filters=32,
                                         kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                         use_bias=True, kernel_size=[5, 1], padding="same", activation=None,
                                         strides=[5, 1],
                                         name='conv1')
            conv1_rgb_bn1 = tf.layers.batch_normalization(conv1_rgb, training=(mode == tf.estimator.ModeKeys.TRAIN),
                                                     name='conv_bn1')
            conv1_rgb_act1 = tf.nn.selu(conv1_rgb_bn1, name="conv_act1")

        with tf.variable_scope('conv1_flow'):
            conv1_u = tf.layers.conv2d(inputs=input_u, filters=32,
                                       kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                       use_bias=True, kernel_size=[5, 1], padding="same", activation=None,
                                       strides=[5, 1],
                                       name='conv1u')
            conv1_u_bn1 = tf.layers.batch_normalization(conv1_u, training=(mode == tf.estimator.ModeKeys.TRAIN),
                                                       name='conv_bn1u')
            conv1_u_act1 = tf.nn.selu(conv1_u_bn1, name="conv_act1u")

            conv1_v = tf.layers.conv2d(inputs=input_v, filters=32,
                                       kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                       use_bias=True, kernel_size=[5, 1], padding="same", activation=None,
                                       strides=[5, 1],
                                       name='conv1v')
            conv1_v_bn1 = tf.layers.batch_normalization(conv1_v, training=(mode == tf.estimator.ModeKeys.TRAIN),
                                                       name='conv_bn1v')
            conv1_v_act1 = tf.nn.selu(conv1_v_bn1, name="conv_act1v")

        input_data = tf.concat([conv1_rgb_act1, conv1_u_act1, conv1_v_act1], axis=1)

        conv2 = tf.layers.conv2d(inputs=input_data, filters=16,
                                 kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                 use_bias=True, kernel_size=[5, 1], padding="same", activation=None, strides=[5, 1],
                                 name='conv2')
        ...
        conv3 = tf.layers.conv2d(inputs=conv_act2, filters=8,
                                 kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                 use_bias=True, kernel_size=[3, 1], padding="same", activation=None, strides=[1, 1],
                                 name='conv3')
        ...
        tf.summary.scalar('loss', eval_loss)
...
def classify(train_records, test_records, num_train, num_test, num_test_samples_per_video, shape):
    ...
    for i in range(1, total_epoch + 1):
        ...
        for j in range(1, int(math.ceil(num_train / batch_size)) * train_epoch + 1):
            ...
            if j % 100 == 0:
                ...
                train_loss = 0
        print("Video Clips: Training epoch", i * train_epoch, "finished, the avg loss is", total_train_loss / j)
        ...
        for j in range(1, int(math.ceil(num_test / num_samples_per_test_video)) + 1):
