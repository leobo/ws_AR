...
def nn_classifier(train_input_path, eval_input_path, global_step, params):
    with tf.device('/gpu:1'):
        ...
        conv1 = tf.layers.conv2d(inputs=input_rgb, filters=16,
                                 ...
                                 use_bias=True, kernel_size=[16, 1], padding="same", activation=None,
                                 strides=[16, 1],
                                 name='conv1')
        ...
        conv4 = tf.layers.conv2d(inputs=input_flow, filters=16,
                                 kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                 use_bias=False, kernel_size=[20, 1], padding="same", activation=None, strides=[20, 1],
                                 name='conv4')
        ...
        conv4_a = tf.layers.conv2d(inputs=conv_act4, filters=16,
                                 ...
                                 use_bias=False, kernel_size=[16, 1], padding="same", activation=None, strides=[16, 1],
                                 name='conv4_a')
        conv_bn4_a = tf.layers.batch_normalization(conv4_a, training=(mode == tf.estimator.ModeKeys.TRAIN), name='conv_bn4_a')
        conv_act4_a = tf.nn.relu(conv_bn4_a, name="conv_act4_a")
        ...
        res = tf.concat([conv_act3, conv_act6], axis=2)

        flat_rgb = tf.reshape(res, [-1, 2 * 2048 * 4], name='flat_rgb')
...
def classify(train_records, test_records, num_train_samples, num_test_samples, num_test_samples_per_video, num_trans_m,
    ...
    with tf.device('/cpu:1'):
        ...
        for i in range(0, total_epoch + 1, train_epoch):
            ...
            a = int(math.ceil(num_test_samples / batch_size)) + 1
