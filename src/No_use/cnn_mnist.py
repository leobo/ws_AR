...
def cnn_model_fn(features, labels, mode):
    ...
    input_layer = tf.reshape(features["x"], [-1, 320, 240, 1])
...
def read_surfWS_tar(path):
    ...
    t = len(surfWS_path)
    surfWS_des = np.zeros(shape=(t, 240, 320), dtype=np.float32)
    for i in range(t):
        ...
        if x != 240 or y != 320:
            print(surfWS_path[i])
            continue
        surfWS_des[i] = preprocessing.normalize(tem, norm='l2')
...
def main():
    ...
    des, tar = read_surfWS_tar("/home/boy2/UCF101/ws_over_TD")
