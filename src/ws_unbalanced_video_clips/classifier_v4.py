...
def input_fn(train_input_path, eval_input_path):
    ...
    train_dataset = train_dataset.shuffle(buffer_size=9537*3)
...
def nn_classifier(train_input_path, eval_input_path, global_step, params):
    with tf.device('/gpu:1'):
        ...
        conv1 = tf.layers.conv2d(inputs=tf.transpose(tf.reshape(input_data[:, :, :, 0], [-1, 3, 2048, 1]), [0,1,3,2]),
                                 filters=2048,
                                 kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                 use_bias=True, kernel_size=[3, 1], padding="same", activation=None, strides=[3, 1],
                                 name='conv1')
        ...
        conv3 = tf.layers.conv2d(inputs=tf.transpose(tf.reshape(input_data[:, :, :, 1], [-1, 3, 2048, 1]), [0,1,3,2]),
                                 filters=2048,
                                 kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                 use_bias=True, kernel_size=[3, 1], padding="same", activation=None, strides=[3, 1],
                                 name='conv3')
        ...
        conv5 = tf.layers.conv2d(inputs=tf.transpose(tf.reshape(input_data[:, :, :, 2], [-1, 3, 2048, 1]), [0,1,3,2]),
                                 filters=2048,
                                 kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                 use_bias=True, kernel_size=[3, 1], padding="same", activation=None, strides=[3, 1],
                                 name='conv5')
        ...
        flat_rgb = tf.reshape(conv_act1, [-1, 1*2048], name='flat_rgb')
        flat_u = tf.reshape(conv_act3, [-1, 1*2048], name='flat_u')
        flat_v = tf.reshape(conv_act5, [-1, 1*2048], name='flat_v')
        with tf.device('/gpu:0'):
            # fc1 = tf.layers.dense(inputs=flat_rgb, units=1024, activation=None, use_bias=True, name='fc1')
            ...
            dropoutfc1 = tf.layers.dropout(
                inputs=flat_rgb, rate=dropout_rate, training=(mode == tf.estimator.ModeKeys.TRAIN), name='dropoutfc1')

            # fc2 = tf.layers.dense(inputs=flat_u, units=1024, activation=None, use_bias=True, name='fc2')
            ...
            dropoutfc2 = tf.layers.dropout(
                inputs=flat_u, rate=dropout_rate, training=(mode == tf.estimator.ModeKeys.TRAIN), name='dropoutfc2')
            ...
            #                                        name='fc_bn3')
            # fc_act3 = tf.nn.relu(fc_bn3, name="fc_act3")
            dropoutfc3 = tf.layers.dropout(
                inputs=flat_v, rate=dropout_rate, training=(mode == tf.estimator.ModeKeys.TRAIN), name='dropoutfc3')

        # logits_rgb = tf.layers.dense(inputs=dropoutfc1, units=num_unique_classes, use_bias=False, name='logits_rgb')
        # logits_u = tf.layers.dense(inputs=dropoutfc2, units=num_unique_classes, use_bias=False, name='logits_u')
        # logits_v = tf.layers.dense(inputs=dropoutfc3, units=num_unique_classes, use_bias=False, name='logits_v')
        # logits = tf.reduce_mean(tf.stack([logits_rgb, logits_u, logits_v], axis=1), axis=1)

        data = tf.reduce_mean(tf.stack([dropoutfc1, dropoutfc2, dropoutfc3], axis=1), axis=1)
        logits = tf.layers.dense(inputs=data, units=num_unique_classes, use_bias=False, name='logits')
