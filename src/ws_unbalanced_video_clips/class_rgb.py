...
def nn_classifier(train_input_path, eval_input_path, global_step, params):
    with tf.device('/gpu:1'):
        ...
        input_data = tf.reshape(input_rgb, [-1, params['rgb_feature_shape'][2], 2048, 1])
        ...
        conv2 = tf.layers.conv2d(inputs=conv_act1, filters=4,
                                 kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                 use_bias=True, kernel_size=[16, 1], padding="same",
                                 activation=None, strides=[16, 1],
                                 name='conv2')
        ...
        flat_rgb = tf.reshape(conv_act2, [-1, 1 * 2048 * 4], name='flat_rgb')
