...
def nn_classifier(train_input_path, eval_input_path, global_step, params):
    with tf.device('/gpu:1'):
        ...
        conv1 = tf.layers.conv2d(inputs=tf.transpose(input_rgb, [0, 1, 3, 2]), filters=2048,
        ...
        conv2 = tf.layers.conv2d(inputs=conv_act1, filters=2048,
                                 kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                 use_bias=True, kernel_size=[1, 2], padding="same", activation=None, strides=[1, 1],
                                 name='conv2')
        ...
        pool2 = tf.layers.average_pooling2d(inputs=conv_act2, pool_size=[1, 2], strides=[1, 1])
        ...
        flat_rgb = tf.reshape(pool2, [-1, 1 * 2048 * 1 * 1], name='flat_rgb')
