...
def gen_subvideo(data, steps):
    ...
    return np.array([d[np.random.randint(0, len(d))] for d in data_par])
...
def input_gen_all(rgb_feature_path, u_feature_path, v_feature_path, selected_frames_path, video_names,
                  video_labels,
                  seed, pp, steps=5, dim=2, train=False):
    ...
    u_path = [os.path.join(u_feature_path, n + '.mat') for n in video_names]
    v_path = [os.path.join(v_feature_path, n + '.mat') for n in video_names]
    ...
    for rp, up, vp, sf, name, l in zip(rgb_path, u_path, v_path, selected_frames, video_names,
                                       video_labels):
        rgb = np.load(rp)
        u = np.transpose(np.mean(loadmat(up)['data_port_org'], axis=1)[pp])
        v = np.transpose(np.mean(loadmat(vp)['data_port_org'], axis=1)[pp])
...
def crop_main(rgb_frame_feature_list, u_frame_feature_list, v_frame_feature_list, ucf_resNet_train_path,
              ucf_resNet_test_path, train_test_splits_save_path, dims, selected_frames_path, seeds, num_test_sample,
              dataset='ucf'):
    if dataset == 'hmdb':
        ...
        for m in seeds:
            for rgb, u, v in zip(rgb_frame_feature_list, u_frame_feature_list, v_frame_feature_list):
                ...
                for _ in range(num_sample_train):
                    t_rgb, t_u, t_v, t_label = input_gen_all(rgb, u, v, selected_frames_path,
                                                             tts.train_data_label[i]['data'],
                                                             encoder.fit_transform(tts.train_data_label[i]['label']),
                                                             m, pp, steps=train_steps, dim=dims)
                ...
                for _ in range(num_sample_test):
                    e_rgb, e_u, e_v, e_label = input_gen_all(rgb, u, v, selected_frames_path,
...
if __name__ == '__main__':
    ...
    featureStorePath = ["/home/boy2/UCF101/ucf101_dataset/frame_features/resNet_resize",
                        "/home/boy2/UCF101/ucf101_dataset/features/mulNet_feature/u/o",
                        "/home/boy2/UCF101/ucf101_dataset/features/mulNet_feature/v/o"]
    ...
    #     tran_m = Weightedsum('tran_m', [], None).transformation_matrix_gen(dims, train_steps, seed)
    #     tran_m_list.append(tran_m)
    #     tran_m_list_norm.append(tran_m/np.max(tran_m))
    ...
    loc = np.random.uniform(-0.5, 0.5, size=[num_trans_matrices, dims])
    loc.sort()
    ...
    np.save('/home/boy2/UCF101/ucf101_dataset/frame_features/seed.npy', tran_m_list_norm)
    ...
    max_value, test_gt = crop_main(
        [ucf_resNet_crop_save_path_v1, ucf_resNet_crop_save_path_v3, ucf_resNet_crop_save_path_v4,
         featureStorePath[0], featureStorePathFlip[0], ucf_resNet_crop_save_path_v2][3:5],
        [ucf_resNet_flow_crop_save_path_1_v1, ucf_resNet_flow_crop_save_path_1_v3,
         ucf_resNet_flow_crop_save_path_1_v4, featureStorePath[1], featureStorePathFlip[1],
         ucf_resNet_flow_crop_save_path_1_v2][3:5],
        [ucf_resNet_flow_crop_save_path_2_v1, ucf_resNet_flow_crop_save_path_2_v3,
         ucf_resNet_flow_crop_save_path_2_v4, featureStorePath[2], featureStorePathFlip[2],
         ucf_resNet_flow_crop_save_path_2_v2][3:5],
        ucf_resNet_train_path_v1, ucf_resNet_test_path_v1,
        ucf_train_test_splits_save_path, dims, selected_frames_path, tran_m_list_norm, num_sample_test)
    ...
    accuracy = classify(
        ...
        [os.path.join(ucf_resNet_train_path_v1, "rgb_flow_labels_" + str(i) + ".tfrecord") for i in
         range(num_frame_features * num_trans_matrices)],
        ...
        num_train_data * num_frame_features * num_trans_matrices,
