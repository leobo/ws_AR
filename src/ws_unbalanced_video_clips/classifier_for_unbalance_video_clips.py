...
def nn_classifier(train_input_path, eval_input_path, global_step, params):
    with tf.device('/gpu:1'):
        ...
        # input_rgb = tf.reshape(input_rgb, [-1, params['rgb_feature_shape'][0] * params['rgb_feature_shape'][2],
        #                                    params['rgb_feature_shape'][1], 1])
        # input_flow = tf.transpose(input_flow, [0, 1, 3, 2])
        # input_flow = tf.reshape(input_flow, [-1, params['flow_feature_shape'][0] * params['flow_feature_shape'][2],
        #                                      params['flow_feature_shape'][1], 1])
        # input_data = tf.concat([input_rgb, input_flow], axis=2)
        ...
        # input_data = tf.reshape(input_data, [-1, 3, 2, 2048, 1])

        # input_data1 = tf.concat([tf.reshape(input_rgb,
        #                                     [-1, 1, params['rgb_feature_shape'][1] * params['rgb_feature_shape'][2],
        #                                      1]),
        #                          tf.reshape(input_flow, [-1, 2, params['flow_feature_shape'][1] *
        #                                                  params['flow_feature_shape'][2], 1])], axis=1)

        conv1 = tf.layers.conv2d(inputs=tf.transpose(input_rgb, [0, 3, 2, 1]), filters=8,
                                 kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                 use_bias=True, kernel_size=[1, 1], padding="same", activation=None, strides=[1, 1],
                                 name='conv1')
        ...
        pool2 = tf.layers.average_pooling2d(inputs=conv_act2, pool_size=[3, 1], strides=[1, 1])

        conv3 = tf.layers.conv2d(
            inputs=tf.transpose(tf.reshape(input_flow[:, 0, :, :], [-1, 1, params['flow_feature_shape'][1],
                                                                    params['flow_feature_shape'][2]]), [0, 3, 2, 1]),
            filters=8,
            kernel_initializer=tf.contrib.layers.xavier_initializer(),
        ...
        conv4 = tf.layers.conv2d(inputs=conv_act3, filters=4,
                                 kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                 use_bias=False, kernel_size=[3, 1], padding="same", activation=None, strides=[1, 1],
                                 name='conv4')
        ...
        pool4 = tf.layers.average_pooling2d(inputs=conv_act4, pool_size=[3, 1], strides=[1, 1])
        ...
        pool6 = tf.layers.average_pooling2d(inputs=conv_act6, pool_size=[3, 1], strides=[1, 1])

        flat_rgb = tf.reshape(pool2, [-1, 1 * 2048 * 4 * 1], name='flat_rgb')
        flat_u = tf.reshape(pool4, [-1, 1 * 2048 * 4 * 1], name='flat_u')
        flat_v = tf.reshape(pool6, [-1, 1 * 2048 * 4 * 1], name='flat_v')
        with tf.device('/gpu:0'):
            fc1 = tf.layers.dense(inputs=flat_rgb, units=2048, activation=None, use_bias=True, name='fc1')
            ...
            fc2 = tf.layers.dense(inputs=flat_u, units=2048, activation=None, use_bias=True, name='fc2')
            ...
            fc3 = tf.layers.dense(inputs=flat_v, units=2048, activation=None, name='fc3')
        ...
        logits_rgb = tf.layers.dense(inputs=dropoutfc1, units=num_unique_classes, name='logits_rgb')
...
def classify(train_records, test_records, num_train_samples, num_test_samples, num_test_samples_per_video, rgb_shape,
             flow_shape):
